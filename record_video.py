...
import pwd
import pandas as pd
import eclc_temp_utils as bb_utils
...
# @@@ temporarily don't save to file, for easier debugging
# logging.basicConfig(encoding='utf-8',format='%(filename)s %(asctime)s: %(message)s', filemode='a', level=logging.DEBUG)
...
def picam2_record_mp4(filename, outdir, recording_time, fps, shutter_speed, width, height, tuning_file, noise_reduction_mode, digital_zoom): #imformat="yuv" #have excluded imformat input because right now only functions by grabbing YUV frames, then converts them to RGB video. Maybe have a grayscale vs color option if possible?
    ...
    print("beginning video capture in picam2_record_mp4()")
    ...
    print(output)
    vid_fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # Better compatibility for MP4
    out = cv2.VideoWriter(output, vid_fourcc, 5, (width, height))

    for i, im_array in enumerate(frames_list):
        frame = im_array[0]
        if frame is None or frame.size == 0:
            print(f"Frame {i} is invalid")
            continue
        try:
...
def picam2_record_mjpeg(filename, outdir, recording_time, quality, fps, shutter_speed, width, height, tuning_file, noise_reduction_mode, digital_zoom, imformat="RGB888", buffer_count=2):
    ...
    print("Frame duration:",frame_duration_microseconds)
...
def create_todays_folder(dirpath):
    """ Create a folder with today's date in requested path.
    ...
    return bb_utils.ensure_folder_exists(dirpath=dirpath, folder_name=today)
...
def trackTagsFromVid(filepath, todays_folder_path, filename, tag_dictionary, box_type, now):
    ...
    tag_dictionary = cv2.aruco.getPredefinedDictionary(tag_dictionary)
    print("got predefined dictionary")

    # Use DetectorParameters directly (since DetectorParameters_create() is unavailable)
    parameters = cv2.aruco.DetectorParameters()  # Replace DetectorParameters_create
    print(parameters)

    # You can customize the parameters if needed
    # Example: Enable corner refinement for better tag detection
    parameters.cornerRefinementMethod = cv2.aruco.CORNER_REFINE_SUBPIX

    # Further processing (e.g., video frame analysis) goes here
    # Assuming this function will continue to work with the `parameters` object
    # Example placeholder:
    print(f"Tracking tags from video: {filename}")
    # (Your video processing code would continue here...)

    # Create the ArUco detector using the tag dictionary and parameters
    detector = cv2.aruco.ArucoDetector(tag_dictionary, parameters)
    print("Aruco Detector created.")

    # Adjust parameters based on box type
    print(box_type)
    if box_type == 'custom':
        # Change these!
    ...
    if not vid.isOpened:
        print("Vid cannot be opened")
        return
    print(vid)
    print(cv2.CAP_PROP_FRAME_COUNT)
    fps = vid.get(cv2.CAP_PROP_FPS)
    print(f" Default FPS of the video: {fps}")
    #fps = 2  
    vid = cv2.VideoCapture(filepath)
    vid.set(cv2.CAP_PROP_FPS, 5.9)

    #print(f"Manually set FPS: {fps}")
    
    vid.set(cv2.CAP_PROP_FRAME_COUNT, 59)
    print(cv2.CAP_PROP_FRAME_COUNT)
    frame_num = 0
    noID = []
    raw = []
    ...
    while vid.isOpened():
        ...
        ret, frame = vid.read()
        if not ret:
            print ("Failed to process frame", frame_num)
            break
        if ret:
            ...
            # Use the ArUco detector to detect markers in the frame
            ...
            for i in range(len(rejectedImgPoints)):
                ...
                noID.append([filename, setup.colony_number, now, frame_num, "X", float(xmean), float(ymean),
                             float(xmean_top_point), float(ymean_top_point)])
            if ids is None:
                print("No marker detected")
            else:
                print("ids is not None")
                print("Detected marker in frame", frame_number)
                for i in range(len(ids)):
                    ...
                    xmean_top_point = (c[0, 0] + c[1, 0]) / 2  # for calculating the top point of the tag
                    ymean_top_point = (c[0, 1] + c[1, 1]) / 2  # for calculating the top point of the tag
                    raw.append([filename, setup.colony_number, now, frame_num, int(ids[i]), float(xmean), float(ymean),
                                float(xmean_top_point), float(ymean_top_point)])
            print(frame_num)
            frame_num += 1
            ...
            print(frame_num)
        print("Total number processed:", frame_num)

        # Save data to CSV files
        df = pd.DataFrame(raw)
        df = df.rename(columns={0: 'filename', 1: 'colony number', 2: 'datetime', 3: 'frame', 4: 'ID', 5: 'centroidX',
                                6: 'centroidY', 7: 'frontX', 8: 'frontY'})
        df.to_csv(todays_folder_path + "/" + filename + '_raw.csv')
        print('saved raw csv')

        df2 = pd.DataFrame(noID)
        df2 = df2.rename(columns={0: 'filename', 1: 'colony number', 2: 'datetime', 3: 'frame', 4: 'ID', 5: 'centroidX',
                                  6: 'centroidY', 7: 'frontX', 8: 'frontY'})
...
def main():
    ...
    if setup.track_recorded_videos == True:
        ...
        if setup.interpolate_data and df is not None and not df.empty:
            df = interpolate(df, setup.max_seconds_gap, setup.actual_frames_per_second)

        if df is not None and not df.empty and setup.calculate_behavior_metrics:
